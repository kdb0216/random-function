...
def aes_round_iteration_filtered_custom(s_bits, t_total, t_prime, keys, round_constants):
    ...
    :return: 输出集合列表（共t_total-t_prime个）、每轮保留数统计、各轮中间输出全量记录
    """
    ...
    all_round_intermediates = defaultdict(set)  # 记录所有轮次的中间输出值 (轮数: 输出值集合)
    #random_sample_count = 2 ** int((s_bits - t_prime/2))
    random_sample_count = 2 ** 12

    # 遍历每个起始轮i（1-based）
    for start_i in range(1, max_start_round + 2):
        print(f"\n========== 开始处理起始轮 i = {start_i} ==========")
        ...
        for round_offset in range(t_prime):
            ...
            # 记录当前轮的所有输出值
            all_round_intermediates[round_num].update(current_io_map.values())
    ...
    return output_sets, all_round_stats, all_round_intermediates
...
def generate_random_s_bit_message(s_bits):
    """生成随机的s比特消息（整数形式）"""
    max_val = 2 ** s_bits - 1
    return random.randint(0, max_val)


def run_full_t_round_iteration(msg, s_bits, t_total, keys, round_constants):
    """
    对随机消息执行完整的t轮迭代，记录每轮中间值
    :return: 每轮的中间值字典 {轮数: 输出值}
    """
    aes_block_bits = 128
    current_val = msg
    round_intermediates = {}

    for round_num in range(1, t_total + 1):
        ...
        round_intermediates[round_num] = output_val
        current_val = output_val

    return round_intermediates


def check_target_intermediates(output_sets, random_intermediates, t_total, t_prime):
    """
    核心校验逻辑：
    - 随机消息第 t'+1 到 t 轮的中间值 → 待校验值列表
    - 原代码第 1 到 t-t' 个output_sets → 目标集合列表
    - 一一对应校验：第t'+k轮值 → 第k个output_set（k=1~t-t'）
    """
    check_results = []
    target_round_start = t_prime  # 随机消息起始校验轮：t'+1
    target_round_end = t_total  # 随机消息结束校验轮：t
    target_set_count = t_total - t_prime+1  # output_sets数量：t-t'

    # 校验轮数和集合数量是否匹配
    assert (target_round_end - target_round_start + 1) == target_set_count, \
        f"校验轮数({target_round_end - target_round_start + 1})与集合数量({target_set_count})不匹配"

    print("\n========== 核心校验结果（第t'+1~t轮值 → 第1~t-t'个output_sets） ==========")
    print(f"t'={t_prime}，t={t_total} → 校验范围：")
    print(f"- 随机消息中间值：第{target_round_start}~{target_round_end}轮")
    print(f"- 原代码输出集合：第1~{target_set_count}个")
    print("-" * 80)
    print(f"{'随机消息轮数':<15} {'中间值':<20} {'对应output_set序号':<20} {'是否存在':<10}")
    print("-" * 80)

    for k in range(target_set_count):
        # 随机消息轮数：t'+1 + k → 第t'+1、t'+2…t轮
        random_round = target_round_start + k
        # 对应output_set序号：1 + k → 第1、2…t-t'个
        set_idx = k  # 列表索引从0开始，对应第k+1个集合
        # 获取待校验值和目标集合
        random_val = random_intermediates[random_round]
        target_set = output_sets[set_idx]
        # 校验是否存在
        exists = random_val in target_set

        check_results.append({
            'random_round': random_round,
            'random_val': random_val,
            'output_set_idx': set_idx + 1,  # 显示序号（1-based）
            'exists': exists,
            'set_size': len(target_set)
        })

        # 打印单条结果
        print(f"{random_round:<15} {random_val:<20} {set_idx + 1:<20} {exists:<10}")
...
if __name__ == "__main__":
    ...
    #     bytes.fromhex("717273747576778798a9bacbdcedfe0f"),
    ...
    # ]
    keys = [secrets.token_bytes(16) for _ in range(t_total)]
    round_constants = [secrets.token_bytes(16) for _ in range(t_total)]

    # ========== 第一步：执行原代码逻辑，获取输出集合和各轮中间值 ==========
    print("========== 执行原代码逻辑 ==========")
    output_sets, round_stats, original_intermediates = aes_round_iteration_filtered_custom(
        s_bits=s,
        t_total=t_total,
        t_prime=t_prime,
        keys=keys,
    ...
    # ========== 第二步：生成随机s比特消息并执行完整t轮迭代 ==========
    print("\n========== 生成随机s比特消息并执行完整t轮迭代 ==========")
    random_msg = generate_random_s_bit_message(s)
    print(f"生成的随机{s}比特消息（十进制）：{random_msg}")
    print(f"生成的随机{s}比特消息（二进制）：{bin(random_msg)[2:].zfill(s)}")

    # 执行完整t轮迭代，获取所有中间值
    random_intermediates = run_full_t_round_iteration(
        msg=random_msg,
        s_bits=s,
        t_total=t_total,
        keys=keys,
    ...
    print(f"\n随机消息{t_total}轮迭代中间值：")
    for round_num in sorted(random_intermediates.keys()):
        print(f"第{round_num}轮：{random_intermediates[round_num]}")

    # ========== 步骤3：核心校验：第t'+1~t轮值 → 第1~t-t'个output_sets ==========
    check_results = check_target_intermediates(
        output_sets=output_sets,
        random_intermediates=random_intermediates,
        t_total=t_total,
        t_prime=t_prime
    )

    # ========== 步骤4：统计结果 ==========
    exists_count = sum(1 for res in check_results if res['exists'])
    total_check = len(check_results)
    print("-" * 80)
    print(f"\n最终统计：")
    print(f"- 总共校验 {total_check} 组（轮数/集合）")
    print(f"- 随机消息中间值存在于对应output_set中的数量：{exists_count}")
    print(f"- 不存在的数量：{total_check - exists_count}")
    print(f"- 存在率：{exists_count / total_check * 100:.2f}%")