...
def aes_round_iteration_filtered(s_bits, t_rounds, keys, round_constants, filter_round):
    """
    单轮过滤的t轮AES迭代加密
    ...
    :param filter_round: 指定执行过滤的轮数（1-based），仅在该轮执行一次过滤
    :return:
        - 最终输入输出映射
        - 每轮保留的输入数统计
    ...
    if filter_round < 1 or filter_round > t_rounds:
        raise ValueError(f"filter_round必须是1~{t_rounds}之间的整数")
    ...
    for round_idx in range(t_rounds):
        ...
        # 过滤逻辑：仅在指定的filter_round轮执行一次过滤
        if round_num == filter_round:
            print(f"执行第 {round_num} 轮过滤：保留输出对应原始输入数≥2的输入")
            # 过滤规则：仅保留输出对应输入数≥2的输入
            keep_inputs = set()
            for out_val, in_vals in output_to_inputs.items():
                if len(in_vals) >= 2:  # 输出对应≥2个输入，保留这些输入
                    keep_inputs.add(out_val)
            current_inputs = keep_inputs
            print(f"第 {round_num} 轮过滤后保留输入数：{len(current_inputs)}")
        else:
            # 非过滤轮数：保留所有输入的输出作为下一轮输入
...
if __name__ == "__main__":
    ...
    print("\n===== 最终输出及对应的原始输入 =====")
    # 只打印前10个最终输出（避免输出过长）
    print("（仅展示前10个最终输出，完整数据可查看final_output_to_originals字典）")
    for i, (final_out, original_inps) in enumerate(sorted(final_output_to_originals.items())[:10]):
