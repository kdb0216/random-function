...
if __name__ == "__main__":
    ...
    s = 16  # 输入/输出比特数
    t = 256  # 总迭代轮数
    start_filter_round = 1  # 从第几轮开始过滤
    end_filter_round = 1    # 过滤结束轮数

    # 生成与原代码相同的密钥和轮常数（固定种子可复现）
    # random.seed(12345)
    ...
    # ========== 第一步：执行原代码的过滤流程，获取过滤后的最终输出列表 ==========
    print("===== 执行原代码的过滤流程 =====")
    final_io_map, round_keep_counts = aes_round_iteration_filtered(
        s, t, keys, round_constants, start_filter_round, end_filter_round
    )
    filtered_outputs = list(final_io_map.values())  # 原代码过滤后的输出列表
    print(f"原代码过滤后的输出列表长度：{len(filtered_outputs)}")

    # ========== 第二步：随机生成一个s比特值，执行完整t轮迭代 ==========
    print("\n===== 随机生成s比特值并执行完整迭代 =====")
    max_s_val = 2 ** s - 1
    random_input = random.randint(0, max_s_val)  # 随机生成s比特值
    print(f"随机生成的{s}比特输入值：{random_input} (十六进制: 0x{random_input:04X})")

    # 执行完整t轮迭代（无过滤）
    final_output = single_input_full_iteration(random_input, s, t, keys, round_constants)
    print(f"经过{t}轮迭代后的最终输出值：{final_output} (十六进制: 0x{final_output:04X})")

    # ========== 第三步：检查最终输出是否在过滤后的列表中 ==========
    print("\n===== 结果检查 =====")
    if final_output in filtered_outputs:
        print(f"✅ 最终输出值 {final_output} 存在于原代码过滤后的输出列表中")
    else:
        print(f"❌ 最终输出值 {final_output} 不存在于原代码过滤后的输出列表中")

    # 可选：打印过滤后的输出列表（数量大时建议注释）
    # print(f"\n原代码过滤后的输出列表：{filtered_outputs}")